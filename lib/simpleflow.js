...
var run = function(config, params, debug, find_method, cb) {
	...
	console.log('run', JSON.stringify(config) );
	
    var wires = config.wires,
		  modules = config.containers;
	 ...
	 var step = function() {
        ...
        // List modules that must be executed
          ...
          if( !execValues[mId] && pendingModules.indexOf(mId) == -1 ) { // don't re-execute modules
            ...
            if( !wires.some(function(w) { return (w["tgt"]["container"] == mId) && !execValues[ w["src"]["container"] ]; }) ) {
		  ...
		  if(moduleIdsToExecute.length === 0) {
				...
				for(var k = 0 ; k < modules.length ; k++) {
					var m = modules[k];
					if(m.containerType == "output") {
						wires.forEach(function(w) { 
							if(w["tgt"]["container"] == k && execValues[w["src"]["container"]]) {
					          outputValues[m.config["value"]["name"]] = execValues[w["src"]["container"]][ w["src"]["terminal"] ];
				           }
						});
					}
				}
			...
			moduleIdsToExecute.forEach(function(moduleId) {
	         ...
	         // default form values
				for(key in m.config["value"]) { p[key] = m.config["value"][key]; }

				// console.log("Wires: "+JSON.stringify(wires) );

	         // incoming wire params
				wires.filter(function(w) { return w["tgt"]["container"] == moduleId; }).forEach( function(w) {
					
							console.log("Incoming wire: "+JSON.stringify(w) );

							var key =  w["tgt"]["terminal"];
							var val = execValues[ w["src"]["container"] ][ w["src"]["terminal"] ];
				 ...
				 pendingModules.push(moduleId);
				  ...
				  setTimeout(close(m["containerType"], p, moduleId), 0);
