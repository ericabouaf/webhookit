...
npm.load({}, function (err) {
   if (err) throw err;
	npm.commands.ls(['installed','webhookit'], true, function (err, packages) {
		if (err) throw err;
		for(var k in packages) {
			...
			if( pkg.words.indexOf('active') != -1 ) {
				console.log("Loading WebHookIt module package: "+lib+" version "+version);
				var m = require(s[0]);
				if( typeof m.run == "function" && typeof m.definition == "object") {
					definitions.push(m.definition);
					CoreModules[lib] = m.run;
				}
				else {
					console.log("Wrong WebHookIt package: "+pkg.name);
				}
			}
