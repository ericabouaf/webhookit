exports.init = function(db) {
	...
	User.findById = function(user_id, cb, errb) {
		...
		db.collection('users', function(err, collection) {
			if(err) {errb(err); return; }
			collection.find({_id: user_id } , function(err, cursor) {
				if(err) {errb(err); return; }
	...
	User.prototype = {
		...
		update: function(cb, errb) {
			// TODO:
		},
	
		insert: function(cb, errb) {
			// TODO:
		},
	
		wiringByName: function(name, cb, errb) {
			db.collection('wirings', function(err, collection) {	
				if(err) { errb(err); return; }
				collection.find({ name: name }, function (err, docs) {
					if(err) { errb(err); return; }
	      		docs.toArray(function(err, results) {
						if(err) { errb(err); return; }
						if(results.length == 0) { cb(false); return; }
						cb( results[0].config );
					});
	     		});
			});
